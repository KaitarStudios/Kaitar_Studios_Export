...
-----------------------------------------------------------------
local OG = script.Parent.CamFrame.TextButton
for i,v in ipairs(pointer.Value.Parent.Parent:GetChildren()) do
	--print("---------------------")
	local LargestPart = nil
	for i2,v2 in ipairs(v:GetDescendants()) do
		if v2:IsA("BasePart") or v2:IsA("UnionOperation") or v2:IsA("MeshPart") then
			if LargestPart == nil then
				LargestPart = v2
				--print(v2)
			else
				if v2.Size.X*v2.Size.Y*v2.Size.Z > LargestPart.Size.X*LargestPart.Size.Y*LargestPart.Size.Z  then
					LargestPart = v2
					--print(v2)
				end
			end
	...
	if LargestPart then
		...
		NB.MouseButton1Click:Connect(function()
			print(1)
			script.Parent.Klientdriver2.RemoteEvent3:FireAllClients(LargestPart)
		end)
	end
end
OG:Destroy()
